// AllEntranceEffect.jsx
import React, {
  useEffect, useState, useMemo, useCallback, useRef,
} from "react";
import Parse from "../../parseConfig";
import "./AllEntranceEffect.css";
...
const IC = {
  entrance: (
    <svg width="22" height="22" fill="none" viewBox="0 0 24 24">
      <path d="M13 3L4 12l9 9M4 12h16" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
  ),
  search: (
...
function doCSV(items) {
  ...
  a.download = "entrance_effects.csv"; a.click();
}

function doExcel(tableEl) {
  if (!tableEl) return;
  var uri = "data:application/vnd.ms-excel," + encodeURIComponent(tableEl.outerHTML);
  var a = document.createElement("a"); a.href = uri; a.download = "entrance_effects.xls"; a.click();
}
...
function doPDF(items) {
  ...
  var html = [
    ...
    "td{padding:7px 10px;border-bottom:1px solid #e2e8f0}tr:nth-child(even) td{background:#f0f9ff}",
    "</style></head><body>",
    "<h2>Entrance Effects Export</h2>",
...
function PrivateToggle({ checked, onChange, disabled }) {
  return (
    <div className="ee-toggle-wrap">
      <label className="ee-toggle">
        <input type="checkbox" checked={checked} onChange={onChange} disabled={disabled} />
        <div className="ee-track"><div className="ee-thumb" /></div>
      </label>
      <span className={"ee-toggle-lbl " + (checked ? "on" : "off")}>{checked ? "Yes" : "No"}</span>
    </div>
  );
}
...
function CatBadge({ value }) {
  return (
    <span className={"ee-cat " + catClass(value)}>
      <span className="ee-cat-dot" />{value || "—"}
    </span>
  );
}
...
function EditModal({ item, categories, onClose, onSaved, showToast }) {
  ...
  var [credits,   setCredits]   = useState(gCredits(item) !== null ? String(gCredits(item)) : "");
  ...
  async function handleSave() {
    ...
    try {
      ...
      await item.save();
      showToast("✓ Entrance effect updated");
  ...
  return (
    <div className="ee-overlay" onClick={function(e) { if (e.target === e.currentTarget) onClose(); }}>
      <div className="ee-modal">
        <div className="ee-modal-hdr">
          <div className="ee-modal-title">{IC.edit} Edit Entrance Effect</div>
          <button className="ee-modal-x" onClick={onClose} type="button">{IC.x}</button>
        </div>

        <div className="ee-mfield">
          <label className="ee-mlabel">Object ID</label>
          <input className="ee-minput" value={item.id} readOnly
            style={{ opacity:.45, cursor:"not-allowed", fontFamily:"var(--ee-mono)", fontSize:".74rem" }} />
        </div>

        <div className="ee-mfield">
          <label className="ee-mlabel">Name</label>
          <input className="ee-minput" value={name}
            onChange={function(e) { setName(e.target.value); }} placeholder="Effect name" />
        </div>

        <div className="ee-mfield">
          <label className="ee-mlabel">Category</label>
          {categories.length > 1 ? (
            <select className="ee-mselect" value={category}
            ...
            <input className="ee-minput" value={category}
              onChange={function(e) { setCategory(e.target.value); }}
              placeholder="e.g. sparkle, fade, slide…" />
          )}
        </div>

        <div className="ee-mfield">
          <label className="ee-mlabel">Credits</label>
          <input className="ee-minput" type="number" min="0" step="any" value={credits}
            onChange={function(e) { setCredits(e.target.value); }} placeholder="0" />
        </div>

        <div className="ee-mfield">
          <label className="ee-mlabel">Private Access</label>
          <div
            className={"ee-private-row" + (isPrivate ? " prv-on" : "")}
          ...
          >
            <span className="ee-private-label">
              {isPrivate ? "🔒 Private — Restricted access" : "🌐 Public — Available to all"}
            </span>
            <div className={"ee-big-toggle" + (isPrivate ? " on" : "")}>
              <div className="ee-big-toggle-thumb" />
            </div>
          </div>
        </div>

        <div className="ee-modal-ftr">
          <button className="ee-cancel" onClick={onClose} type="button">Cancel</button>
          <button className="ee-save" onClick={handleSave} disabled={saving} type="button">
...
/* ─────────────────────────────────────────
   MAIN COMPONENT
───────────────────────────────────────── */
export default function AllEntranceEffect() {
  ...
  var fetchData = useCallback(async function() {
    setLoading(true);
    try {
      var q = new Parse.Query(Parse.Object.extend("EntranceEffect"));
  ...
  var handleToggle = useCallback(async function(item) {
    var id = item.id; var current = gPrivate(item);
    ...
    } catch (err) { console.error("Toggle error:", err); showToast("✗ Toggle failed"); }
    finally { setToggling(function(prev) { return Object.assign({}, prev, { [id]: false }); }); }
  }, [showToast]);
  ...
  var handleDelete = useCallback(async function(item) {
    ...
    try {
      ...
      showToast("✓ Entrance effect deleted");
  ...
  var privateCount  = useMemo(function() { return rows.filter(gPrivate).length; }, [rows]);
  var totalCredits  = useMemo(function() { return rows.reduce(function(acc,r) { return acc + (gCredits(r)||0); }, 0); }, [rows]);

  var filtered = useMemo(function() {
    var list = rows;
    if (catFilter !== "All") list = list.filter(function(r) { return gCat(r) === catFilter; });
    if (prvFilter) list = list.filter(gPrivate);
    var q = search.trim().toLowerCase();
    if (!q) return list;
    return list.filter(function(r) {
      return r.id.toLowerCase().includes(q) || gName(r).toLowerCase().includes(q) || gCat(r).toLowerCase().includes(q);
    });
  ...
  return (
    <div className="ee-page">
      <div className="ee-topline" />
      <div className="ee-wrap">

        {/* HEADER */}
        <div className="ee-header">
          <div className="ee-hdr-left">
            <div className="ee-logo">{IC.entrance}</div>
            <div>
              <div className="ee-title">All Entrance Effects</div>
              <div className="ee-sub">Manage entrance effect assets &amp; access controls</div>
            </div>
          </div>
          <div className="ee-chips">
            <div className="ee-chip total">✨ {rows.length} effects</div>
            <div className="ee-chip private">🔒 {privateCount} private</div>
            <div className="ee-chip credits">🪙 {fmtCredits(totalCredits)}</div>
          </div>
        </div>

        {/* TOOLBAR */}
        <div className="ee-toolbar">
          <div className="ee-srch-wrap">
            <span className="ee-srch-ico">{IC.search}</span>
            <input className="ee-srch" type="text" placeholder="Search by ID, name or category…"
              value={search} onChange={function(e) { setSearch(e.target.value); }} />
            {search && (
              <button className="ee-srch-clr" onClick={function() { setSearch(""); }} type="button">{IC.x}</button>
            )}
          </div>

          <div className="ee-filters">
            <button className={"ee-fbtn" + (catFilter === "All" && !prvFilter ? " act" : "")}
              onClick={function() { setCatFilter("All"); setPrvFilter(false); }} type="button">All</button>
            <button className={"ee-fbtn prv" + (prvFilter ? " act" : "")}
              onClick={function() { setPrvFilter(function(v) { return !v; }); setCatFilter("All"); }} type="button">
              🔒 Private Only
            </button>
            {categories.map(function(cat) {
              return (
                <button key={cat} className={"ee-fbtn" + (catFilter === cat ? " act" : "")}
                  onClick={function() { setCatFilter(cat); setPrvFilter(false); }} type="button">{cat}</button>
              );
            })}
          </div>

          <div className="ee-exports">
            <button className="ee-exp copy" type="button"
              onClick={function() { doCopy(filtered); showToast("✓ Copied to clipboard!"); }}>
              {IC.copy} <span>Copy</span>
            </button>
            <button className="ee-exp csv" type="button" onClick={function() { doCSV(filtered); }}>
              {IC.csv} <span>CSV</span>
            </button>
            <button className="ee-exp excel" type="button" onClick={function() { doExcel(tableRef.current); }}>
              {IC.excel} <span>Excel</span>
            </button>
            <button className="ee-exp pdf" type="button" onClick={function() { doPDF(filtered); }}>
              {IC.pdf} <span>PDF</span>
            </button>
            <button className="ee-exp print" type="button" onClick={function() { window.print(); }}>
        ...
        {/* SUMMARY */}
        <div className="ee-summary">
          <div className="ee-info">
            Showing <strong>{startIdx}–{endIdx}</strong> of <strong>{filtered.length}</strong> effects
            {search && ' matching "' + search + '"'}
            {catFilter !== "All" && ' in "' + catFilter + '"'}
            {prvFilter && " · private only"}
          </div>
          <div className="ee-ppwrap">
            <span>Rows:</span>
            <select className="ee-ppsel" value={perPage}
        ...
        <div className="ee-card">
          {loading ? (
            <div className="ee-loading">
              ...
              Loading entrance effects…
            </div>
          ) : filtered.length === 0 ? (
            <div className="ee-empty">
              <div className="ee-empty-ico">✨</div>
              <div className="ee-empty-title">
                {search || catFilter !== "All" || prvFilter ? "No results found" : "No entrance effects yet"}
              </div>
              <div className="ee-empty-desc">
                {search ? 'Nothing matches "' + search + '"'
                  : prvFilter ? "No private effects found"
                  : catFilter !== "All" ? 'No effects in "' + catFilter + '"'
                  : "Entrance effects will appear here once added."}
              </div>
            </div>
          ) : (
            <>
              {/* DESKTOP TABLE */}
              <div className="ee-tbl-scroll">
                <table className="ee-tbl" ref={tableRef}>
                  <thead>
                    <tr>
                      <th>#</th><th>Object ID</th><th>Date</th><th>Name</th>
                      <th>Category</th><th>Credits</th><th>Private</th><th>Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                    {pageItems.map(function(item, idx) {
                      var nm  = gName(item); var cat = gCat(item);
                      var cred = gCredits(item); var prv = gPrivate(item);
                      return (
                        <tr key={item.id}>
                          <td><span className="ee-num">{startIdx + idx}</span></td>
                          <td>
                            <span className="ee-oid" title={"Click to copy: " + item.id}
                              onClick={function() {
                                if (navigator.clipboard) navigator.clipboard.writeText(item.id);
                                showToast("✓ Object ID copied!");
                              }}>{item.id}</span>
                          </td>
                          <td><span className="ee-date">{fmtDate(item.get("createdAt"))}</span></td>
                          <td>
                            <div className="ee-name-cell">
                              <div className="ee-avatar" style={{ background: avatarGrad(nm) }}>{initials(nm)}</div>
                              <span className="ee-name-text">{nm}</span>
                            </div>
                          </td>
                          <td><CatBadge value={cat} /></td>
                          <td><span className="ee-credits">🪙 {fmtCredits(cred)}</span></td>
                          <td>
                            <PrivateToggle checked={prv}
                              onChange={function() { handleToggle(item); }}
                              disabled={!!toggling[item.id]} />
                          </td>
                          <td>
                            <div className="ee-actions">
                              <button className="ee-btn-edit" type="button"
                                onClick={function() { setEditItem(item); }}>{IC.edit} Edit</button>
                              <button className="ee-btn-del" type="button"
              ...
              {/* MOBILE CARDS */}
              <div className="ee-mob-list">
                {pageItems.map(function(item, idx) {
                  var nm = gName(item); var cat = gCat(item);
                  var cred = gCredits(item); var prv = gPrivate(item);
                  return (
                    <div key={item.id} className={"ee-mob-card" + (prv ? " is-private" : "")}>
                      <div className="ee-mob-top">
                        <div className="ee-name-cell">
                          ...
                          <div>
                            <div className="ee-name-text" style={{ fontSize:".88rem" }}>{nm}</div>
                            <div style={{ fontSize:".67rem", color:"var(--ee-t4)", marginTop:1 }}>#{startIdx+idx}</div>
                          </div>
                        </div>
                        <PrivateToggle checked={prv}
                          onChange={function() { handleToggle(item); }}
                      ...
                      <div className="ee-mob-grid">
                        <div className="ee-mob-field">
                          <span className="ee-mob-lbl">Object ID</span>
                          <span className="ee-oid" style={{ maxWidth:"100%", fontSize:".67rem" }} title={item.id}
                            onClick={function() {
                              if (navigator.clipboard) navigator.clipboard.writeText(item.id);
                              showToast("✓ Copied!");
                            }}>{item.id}</span>
                        ...
                        <div className="ee-mob-field">
                          <span className="ee-mob-lbl">Date</span>
                          <span className="ee-mob-val">{fmtDate(item.get("createdAt"))}</span>
                        </div>
                        <div className="ee-mob-field">
                          <span className="ee-mob-lbl">Category</span>
                          <CatBadge value={cat} />
                        </div>
                        <div className="ee-mob-field">
                          <span className="ee-mob-lbl">Credits</span>
                          <span className="ee-credits" style={{ fontSize:".82rem" }}>🪙 {fmtCredits(cred)}</span>
                        </div>
                      </div>
                      <div className="ee-mob-footer">
                        <button className="ee-btn-edit" type="button"
                          onClick={function() { setEditItem(item); }}>{IC.edit} Edit</button>
                        <button className="ee-btn-del" type="button"
                          onClick={function() { handleDelete(item); }}>{IC.trash} Delete</button>
          ...
          {!loading && filtered.length > 0 && (
            <div className="ee-footer">
              <div className="ee-foot-info">
                <strong>{startIdx}–{endIdx}</strong> of <strong>{filtered.length}</strong> entrance effects
              </div>
              <div className="ee-pages">
                <button className="ee-pg" type="button"
                  onClick={function() { setPage(function(p) { return Math.max(0,p-1); }); }}
                  disabled={safePage === 0} aria-label="Previous">{IC.prev}</button>
                {pageNums.map(function(p,i) {
                  if (p === "…") return <button key={"el"+i} className="ee-pg" disabled type="button">…</button>;
                  return (
                    <button key={p} className={"ee-pg" + (safePage===p?" on":"")}
                      onClick={function() { setPage(p); }} type="button">{p+1}</button>
                  );
                })}
                <button className="ee-pg" type="button"
                  onClick={function() { setPage(function(p) { return Math.min(totalPages-1,p+1); }); }}
                  disabled={safePage >= totalPages-1} aria-label="Next">{IC.next}</button>
              </div>
            </div>
          )}
        </div>
      ...
      {editItem && (
        <EditModal item={editItem} categories={categories}
          onClose={function() { setEditItem(null); }}
          onSaved={handleSaved} showToast={showToast} />
      )}

      {toast && <div className="ee-toast">{toast}</div>}
    </div>
  );
}